import { recompraSteps } from "@/lib/recompra-data";
import type { RecompraStatus } from "@/lib/recompra-data";
import { Check } from "lucide-react";

export function RecompraStepper({ current }: { current: RecompraStatus }) {
  const idx = recompraSteps.indexOf(current);
  const activeIdx = idx === -1 ? 0 : idx;

  return (
    <div className="flex items-start gap-1 overflow-x-auto pb-1">
      {recompraSteps.map((step, i) => {
